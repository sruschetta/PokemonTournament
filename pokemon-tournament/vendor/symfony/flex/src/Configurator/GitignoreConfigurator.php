...
class GitignoreConfigurator extends AbstractConfigurator
{
    ...
    {
        $this->write('Adding entries to .gitignore.old');

        $gitignore = $this->options->get('root-dir').'/.gitignore.old';
    ...
    {
        $file = $this->options->get('root-dir').'/.gitignore.old';
        if (!file_exists($file)) {
            return;
        }
        ...
        $this->write('Removing entries in .gitignore.old');
